window.onload = function() {
    ...
    const defaultLiffId = "1655429670-J7pqYNnz";   // change the default LIFF value if you are not using a node server
...
function registerButtonHandlers() {
    document.getElementById('openWindowButton').addEventListener('click', function() {
        liff.openWindow({
            url: 'https://pesenmakan.herokuapp.com/', // Isi dengan Endpoint URL aplikasi web Anda
            external: true
        });
    });
}
...
document.getElementById('sendMessageButton').addEventListener('click', function() {
    if (!liff.isInClient()) {
        ...
        liff.sendMessages([{
            ...
            window.alert('Ini Send Message, pesen apa aja kamu?');
        }).catch(function(error) {
            window.alert('Error lurr!: ' + error);
        });
    }
});
